		...
		<UContainer>
			<div class="max-w-4xl mx-auto space-y-6">
				...
				<div>
					...
					<p>
						Ajoutez une release manuellement avec tous ses détails et musiques
					</p>
				</div>
				...
				<UCard class="bg-cb-quinary-900">
					...
					>
						...
						<UFormField label="ID YouTube Music" name="id_youtube_music" required>
							...
							/>
							<template #help>
								<p class="text-xs text-gray-500">L'identifiant unique de la release sur YouTube Music (obligatoire)</p>
							</template>
						</UFormField>
				...
				<UCard v-if="createdRelease">
					...
					<div v-if="musics.length > 0" class="mb-6">
						<h4 class="text-sm font-medium text-gray-700 mb-3">Musiques dans la release :</h4>
						<div class="space-y-3">
							<div
							...
							>
								<div class="flex items-center space-x-3">
									<div class="flex items-center justify-center w-8 h-8 bg-primary-100 text-primary-600 rounded-full text-sm font-medium">
										{{ index + 1 }}
									</div>
									<div>
										...
										<p class="text-sm text-gray-500">
											...
											<span v-if="music.verified" class="text-primary-500 ml-2">• Vérifié</span>
										</p>
									</div>
								</div>
								<UButton
					...
					<div class="space-y-4">
						<div class="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
							...
							<h4 class="text-lg font-medium text-gray-900 mb-2">Ajouter des musiques</h4>
							<p class="text-gray-500 mb-4">Recherchez et ajoutez des musiques existantes à cette release</p>
							...
							<div v-if="musicOptions.length > 0" class="mt-4 max-w-2xl mx-auto">
								<div class="bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
									<button
										v-for="music in musicOptions"
										:key="music.id"
										type="button"
										class="w-full px-4 py-3 text-left hover:bg-gray-50 flex items-center justify-between border-b border-gray-100 last:border-b-0"
							...
							<div v-else-if="musicSearchQuery.length > 2 && !isSearchingMusic" class="mt-4 text-center">
								<p class="text-sm text-gray-500">Aucune musique trouvée pour "{{ musicSearchQuery }}"</p>
								<p class="text-xs text-gray-400 mt-1">Seules les musiques existantes peuvent être ajoutées</p>
							</div>
						</div>
					</div>
				</UCard>
