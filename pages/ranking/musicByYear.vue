	...
	<div class="container mx-auto space-y-5 p-5">
		<div class="space-y-2">
			<div class="grid grid-cols-1 gap-2 md:grid-cols-2">
				<UInput v-model="search" placeholder="Recherche par nom de musique" />
				<ArtistAlgoliaSelect v-model="selectedArtist" class="" />
			</div>

			<div class="flex flex-col justify-between gap-2 lg:flex-row">
				<div class="flex flex-col gap-2 lg:flex-row">
					<UInput v-model="selectedYear" placeholder="Année (ex: 2024)" type="number" />
					...
					<UButton @click="toggleOrderDirection" color="neutral" variant="outline">
						...
						/>
						Trier par date :
						{{ orderDirection === 'desc' ? 'Plus récentes' : 'Plus anciennes' }}
					</UButton>
				</div>
			...
			<UCheckbox v-model="isMv" label="Afficher uniquement les clips (isMv)" class="" />
		</div>
		<div class="space-y-2">
