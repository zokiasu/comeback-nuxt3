...
<template>
	<div>
		<div v-if="music && music?.artists">
			<button
				class="group bg-quinary-900 relative aspect-square max-h-96 w-full overflow-hidden rounded-lg drop-shadow-lg"
				@click="playVideo(music.id_youtube_music)"
			>
				<div v-if="music.thumbnails?.length" class="relative h-full w-full">
					<div
						class="bg-quinary-900 absolute inset-0 h-full w-full transition-all duration-500 ease-in-out"
				...
				<div
					class="bg-quinary-900/70 absolute inset-0 flex flex-col justify-between p-2 lg:p-3"
				>
					<div class="space-y-1 text-left">
						<NuxtLink
							v-if="music.name"
							:to="'/release/' + music.id"
							class="hover:text-primary-900 font-semibold transition-all duration-300 ease-in-out lg:text-xl"
						>
							{{ music.name }}
						</NuxtLink>
					...
					<div class="flex justify-end">
						<IconPause
							...
							class="group-hover:text-primary-900 h-8 w-8 transition-all duration-500 ease-in-out md:h-10 md:w-10"
						/>
						<IconPlay
							v-else
							class="group-hover:text-primary-900 h-8 w-8 transition-all duration-500 ease-in-out md:h-10 md:w-10"
		...
		<SkeletonDefault
			v-else
			class="bg-quinary-900 aspect-square h-full max-h-96 w-full rounded-lg drop-shadow-lg"
		/>
	</div>
</template>
