...
<template>
  <CardDefault name="Comeback reported" :class="{ hidden: !newsT }">
    <div class="grid grid-cols-1 gap-2 py-5 md:grid-cols-2 xl:grid-cols-3">
      <CardNews
    ...
    <button
      v-if="newsT.length > 9 && newsT.length != maxDisplay"
      class="w-full text-center font-semibold"
      @click="maxDisplay = newsT.length"
    >
      <p>See More</p>
      <IconArrowDown class="mx-auto h-5 w-5" />
    </button>
    <button
      v-if="newsT.length == maxDisplay"
      class="w-full text-center font-semibold"
      @click="maxDisplay = 9"
    >
