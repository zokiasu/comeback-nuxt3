...
<template>
	...
	>
		<button
			...
			class="bg-quaternary-950 col-span-1 flex w-full items-center gap-2 rounded p-2 px-3"
			:class="{
				'group hover:bg-tertiary-200/10': idYoutubeVideo != musicId,
		...
		>
			<div class="hidden w-10 shrink-0 md:block">
				<NuxtImg
					...
					:alt="musicName"
					:src="musicImage"
					class="shadow-secondary-950 h-10 w-10 rounded shadow"
				/>
			</div>
			<div class="min-w-0 flex-1 overflow-hidden">
				...
				>
					<NuxtImg
						...
						:alt="artistName"
						:src="artistImage"
						class="shadow-secondary-950 h-3 w-3 rounded-full object-cover shadow"
					/>
					<NuxtLink
		...
		<button
			v-if="ismv"
			class="bg-primary-900 hover:bg-primary-900/50 flex w-full items-center justify-center rounded px-2 py-1 text-xs font-semibold tracking-widest uppercase"
		...
		>
			...
			<p class="text-tertiary-200/80 hover:text-tertiary-200 cursor-pointer">Close M/V</p>
		</div>
	</div>
</template>
