...
<template>
	...
	>
		<section
			v-if="artists"
			class="bg-quinary-900 h-full w-full shrink space-y-1 overflow-hidden p-2"
		>
			<div class="flex flex-wrap gap-1">
				...
				>
					...
					<h2
						class="group-hover:text-primary-900 truncate text-xs font-semibold transition-all duration-300 ease-in-out lg:text-sm"
					>
						{{ artistObject.name }}
						<span
							v-if="artists.length > 1 && index != artists.length - 1"
							class="group-hover:text-tertiary-200"
						>
							,
						</span>
					</h2>
		...
		<section
			class="bg-quaternary-950 -mt-0.5 flex min-w-[18%] items-center justify-center px-3 py-1 text-center md:mt-0 md:py-0"
		>
			<p
			...
			<p
				...
				class="text-primary-900 my-auto font-medium whitespace-nowrap"
			>
				Today
			</p>
			<p
